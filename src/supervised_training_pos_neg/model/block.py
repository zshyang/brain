...
class PointDescriptor(nn.Module):
    """
    projects face centers into feature space by convolution
    """
...
class NeighborResampleUniform(nn.Module):
    ...
    def forward(self, ring_n=None, neighbor_corners=None):
        ...
        rs_neighbor_corners = torch.cat(
            [neighbor_corners]*self.resampling_frequency, dim=2)
...
class NeighborResampleWeighted(nn.Module):
    ...
    def forward(self, ring_n=None, neighbor_corners=None):
        ...
        rs_ring_n = torch.zeros(
            (num_meshes, num_faces, num_samples), device=device)
        rs_neighbor_corners = torch.zeros(
            (num_meshes, num_faces, num_samples, 3, 3), device=device)
        ...
        perimeter = torch.stack([self.dist_neighbor_corners(neighbor_corners1, neighbor_corners2),
                                 self.dist_neighbor_corners(
                                     neighbor_corners2, neighbor_corners3),
                                 self.dist_neighbor_corners(
                                     neighbor_corners3, neighbor_corners1)
                                 ], dim=3).sum(dim=3)
        ...
        rs_neighbor_corners = torch.cat(
            [rs_neighbor_corners, neighbor_corners], 2)
...
class AlphaBetaGamma(nn.Module):
    ...
    def __init__(self, num_faces=500, num_samples=24, device=None):
        ...
        self.num_samples = num_samples
        self.device = device
        self.alpha = Parameter(torch.rand(
            self.num_faces, self.num_samples), requires_grad=False)
        self.beta = Parameter(torch.rand(
            self.num_faces, self.num_samples), requires_grad=False)
        self.gamma = Parameter(torch.rand(
            self.num_faces, self.num_samples), requires_grad=False)
        self.initialize()
...
class ConvSurface(nn.Module):
    ...
    def __init__(self, num_faces=500, num_neighbor=3, cfg={}):
        ...
        if self.rs_mode == 'Uniform':
            self.neighbor_resample = NeighborResampleUniform(
                self.num_samples_per_neighbor)

        if self.rs_mode == 'Weighted':
            self.neighbor_resample = NeighborResampleWeighted(self.num_samples)

        self.alpha_beta_gamma_nonparam = AlphaBetaGamma(
            self.num_faces, self.num_samples)
    ...
    def get_neighbor_corners(self, faces, verts, ring_n):
        ...
        neighbor_corners = verts[torch.arange(
            num_meshes)[:, None, None, None], neighbor_faces]
    ...
    def points_from_surface(self, rs_neighbor_corners, alpha, beta, gamma):
        ...
        points_neighbor = torch.zeros(
            (num_meshes, num_faces, num_samples, 3), device=device)
        ...
        points_neighbor[meshes_valid] = torch.stack([alpha[:, :, :, None] * corner1,
                                                     beta[:, :, :, None] *
                                                     corner2,
                                                     gamma[:, :, :, None] * corner3], dim=4).sum(dim=4)
    ...
    def forward(self, verts, faces, ring_n, centers):
        ...
        points_neighbor = self.points_from_surface(
            rs_neighbor_corners, alpha, beta, gamma)
