...
def surfaces(mesh, plot):
    ...
    tri = a3.art3d.Poly3DCollection(
        vtx, facecolors=surface_color +'55', 
...
def view_meshes(*files, offset=.2):
    ...
    for file in files:
        ...
        max_y_current = mesh[0][:, 1].max()
        mesh[0][:, 1] += max_y + offset
        mesh[0][:, 2] += 1.0
        ...
        max_x += (max_x_current + offset) * 1.5
        max_y += (max_y_current + offset) * 0.5
