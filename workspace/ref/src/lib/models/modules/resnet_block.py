...
from model.modules.common import ConvType, NormType, conv, get_norm
...
class BasicBlockBase(nn.Module):
    ...
    def __init__(self,
        ...
        self.norm1 = get_norm(self.NormType, planes, D, bn_momentum=bn_momentum)
        self.conv2 = conv(
            planes, planes, kernel_size=3, stride=1, dilation=dilation, bias=False, conv_type=conv_type, D=D)
        self.norm2 = get_norm(self.NormType, planes, D, bn_momentum=bn_momentum)
...
class Bottleneck(BasicBlockBase):
    NORM_TYPE = NormType.BATCH_NORM
