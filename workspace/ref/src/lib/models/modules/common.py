...
def convert_conv_type(conv_type, kernel_size, D):
    ...
    if conv_type == ConvType.SPATIAL_HYPERCUBE:
        ...
        axis_type = [ME.RegionType.HYPERCUBE, ] * 3
        if D == 4:
            axis_type.append(ME.RegionType.HYPERCROSS)
    return region_type, axis_type, kernel_size
...
def conv(in_planes, 
    ...
    kernel_generator = ME.KernelGenerator(
        kernel_size, stride, dilation, region_type=region_type, axis_type=axis_type, dimension=D)
