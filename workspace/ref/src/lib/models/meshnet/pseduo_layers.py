...
class PsuedoConvFace(nn.Module):
    ...
    def forward(self, fea, ring_n, pool_idx):
        ...
        ring_n = ring_n.expand(num_meshes, num_channels, num_faces, -1) # [num_meshes, in_channel, num_faces//2, 1]

        neighbor_fea = fea[
            torch.arange(num_meshes)[:, None, None, None],
            torch.arange(num_channels)[None, :, None, None],
            ring_n
        ] # [num_meshes, in_channel, num_faces//2, 6, 1]
        neighbor_fea = neighbor_fea.squeeze(4) # [num_meshes, in_channel, num_faces//2, 6]

        # Pool input feature only at pool_idx
        # Pooling here occurs at the spatial dimension
        fea = fea[:, :, pool_idx, :] # [num_meshes, in_channel, num_faces//2, 1]

        # Concatenate gathered neighbor features to face_feature, and then find the sum
        fea = torch.cat([fea, neighbor_fea], 3) # [num_meshes, in_channel, num_faces//2, 7]
        # assert fea.shape == (num_meshes, num_channels, num_faces, self.num_neighbor + 1)
        fea = torch.sum(fea, 3) # [num_meshes, in_channel, num_faces//2]
