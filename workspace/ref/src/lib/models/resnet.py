...
from model.modules.common import ConvType, NormType, conv, get_norm, sum_pool
from model.modules.resnet_block import BasicBlock, Bottleneck

T1 = Annotated[BasicBlock, Bottleneck]
...
class ResNetBase(Model):
    ...
    def weight_initialization(self):
        for m in self.modules():
            if isinstance(n, ME.MinkowskiBatchNorm):
                nn.init.constant_(m.bn.weight, 1)
                nn.init.constant_(m.bn.bias, 0)

    def _make_layer(self, block:T1, planes, blocks, stride=1, dilation=1, norm_type=NormType.BATCH_NORM, bn_momentum=0.1):
        downsample = None
